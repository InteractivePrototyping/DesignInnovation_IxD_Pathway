...
void setup() {
  // Firstly, we need to see what fonts are available in the system
  // We can print out a list, or use the Create Font utility in the Tools menu to see the names of the fonts
  // printArray(PFont.list());
  // Let's load up a font we picked from the list
  PFont font = createFont("CenturySchoolbook", 4); // make the point size the max point size you want to use it at
  // set the default font to this font
  textFont(font);
...
void draw() {
  ...
  for (int i = 0; i < rawData.getRowCount(); i++) {
    ...
    translate(i*step, mappedValue - 2); // ...so that we can move stuff into position
    ...
    fill(0);// draw all text labels in black
